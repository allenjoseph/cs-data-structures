...
int suma(int sum, Nodo<int *> *node)
{
    if (node->siguiente != NULL)
    {
        sum = sum + *node->siguiente->elemento;
        return suma(sum, node->siguiente);
    }
...
int main()
{
    ...
    ListaEnlazada<int> *list = new ListaEnlazada<int>();
    ...
    int sum = suma(0, list->primerNodo());
