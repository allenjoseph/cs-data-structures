...
int main()
{
    ...
    ListaEnlazada<int> *triple1 = new ListaEnlazada<int>();
    ListaEnlazada<int> *triple2 = new ListaEnlazada<int>();
    ListaEnlazada<int> *triple3 = new ListaEnlazada<int>();

    for (int num = 1; num <= N; num++)
    {
        int doubleN = num * num;
        triple1->agregar(doubleN);
        triple2->agregar(doubleN);
        triple3->agregar(doubleN);
    }

    ListaEnlazada<int> *result = new ListaEnlazada<int>();

    for (int i = 1; i <= N; i++)
    {
        int double1 = triple1->getValor();

        triple2->moverAInicio();
        for (int j = 1; j <= N; j++)
        {
            int double2 = triple2->getValor();

            triple3->moverAInicio();
            for (int k = 1; k <= N; k++)
            {
                int double3 = triple3->getValor();

                if (double3 == (double1 + double2))
                {
                    result->agregar(i);
                    result->agregar(j);
                    result->agregar(k);
    ...
    int c;
    for (c = 0, result->moverAInicio(); result->posicionActual() < result->longitud(); result->siguiente(), c++)
    {
        if (c > 0 && c % 3 == 0)
        {
            cout << endl;
